...
module.exports = {
  ...
  handlePrefixCommand: async function (message, cmd, settings) {
    ...
    if (cmd.category === "OWNER" && !OWNER_IDS.includes(message.author.id)) {
     ...
     .setDescription("**This command is only for owner**")
     .setColor("#000000")
  ...
  getCommandUsage(cmd, prefix = PREFIX_COMMANDS.DEFAULT_PREFIX, invoke) {
    ...
    const embed = new EmbedBuilder().setFooter({ text: `made by Alive.`}).setColor(EMBED_COLORS.BOT_EMBED).setDescription(desc).setAuthor({ name: `Command Usage`});
